(function () {
  ...
  const saveBtnLabel = saveBtn.textContent || "Save";
  const importBtnLabel = importBtn.textContent || "Import";
  const seasonConfirmBtnLabel = seasonConfirmBtn.textContent || "Confirm";
  ...
  importBtn.addEventListener("click", () => {
    ...
    window
      ...
      .then((res) => {
        ...
        if (type === "players" || type === "visitors" || type === "notes") {
          importResult.textContent = `Created: ${created}, Skipped: ${skipped}${backup}`;
        } else {
          importResult.textContent = `Updated: ${updated}, Not found: ${notFound}${backup}`;
        }
