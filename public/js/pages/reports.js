(function () {
  ...
  function buildFilters(players) {
    const years = new Set([String(state.settings.season || getNowYear()), getNowYear()]);
    players.forEach((player) => {
  ...
  function loadData() {
    window.apiFetch("/reports/access")
      .then(() =>
        Promise.all([
      ...
      .then(([settingsRes, playersRes, visitorsRes]) => {
      ...
      .catch((error) => {
        console.error(error);
        if (window.toast) {
          window.toast(error?.message || "Unable to load reports.", "error");
        }
