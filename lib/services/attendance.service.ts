...
export async function getAttendance(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("id, player_id, session_date, present, created_at, updated_at")
...
export async function postAttendance(request: NextRequest) {
  try {
    ...
    const { data: previous } = await supabase
      .from("player_attendance")
      .select("id, player_id, session_date, present")
